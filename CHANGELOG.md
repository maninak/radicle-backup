...
- A restore into a home holding a key an earlier migration displaced is no longer treated as a restore into an empty one. The check asked for the name the NEXT retirement would take, which is by construction a name nothing is at, so it answered "nothing there" whatever the home held: the one sign of a migrated home this refusal exists to notice could never fire.

- A restore now says when the git it is using checks nothing inside a bundle. Both readers pass `fetch.fsckObjects`, which is the whole of what stops a bundle writing a tree entry named `.git` into storage, and git consults it on a bundle only from 2.46: every older git accepted the setting and ignored it, so a home whose objects nobody had checked came back looking exactly like one whose objects had been. `restore` and the shipped `restore.sh` now both say so, and say which git runs the check.
...
- `doctor` and `restore` agree about what makes two commit ids equal. `doctor` compared byte for byte where `restore` ignores case, so an uppercase row in the node's own record made it report work as stranded on this disk and send you to announce it.
...
- `restore` compares a repository against what other nodes have announced they hold of your signed refs, instead of against your own storage. It was reading back what it had just written: `rad sync <rid> --fetch` over a repository already in storage is a pull, and heartwood's pull deliberately ignores the local peer's key, so the refs read back were always the archive's own. Every restore reported every repository "in step with the network", which is the one answer that means "safe to write", on a comparison of a value with itself. The standings `the network was ahead` and `diverged` are gone, because neither can be established this way: a node holding refs signed with your key that this copy does not have is now reported as `another node holds signed refs this copy does not have`, and it still exits `3`. `in step with the network` is gone for the same reason and is now `no other node has reported holding anything else`: heartwood rewrites its record of a peer only when that peer announces a different head, so disagreement announces itself and agreement is silent, and the report now says so out loud instead of reading silence as a pass. What counts as evidence is a row whose head is not the archive's, whenever some node wrote it: a row that agrees may be one that was already in the table and proves nothing, while a row that differs says the same thing before and after a restore. The run waits twenty seconds after the fetches for other nodes to say what they hold, because a refs announcement is a separate message from the fetch and reading the table microseconds later found nothing on a network that was working fine. `restore --json` renames the `diverged` array to `atRisk`, gains `ahead` (repositories holding work some node has never seen, previously visible only by parsing the standing strings), and turns `notChecked` from a count into the list of repositories it counts.
- `restore` says `holds work no node that answered has` where it said `holds work the network has not seen`. A node that said nothing contributed nothing, which is the same reasoning that turned `in step with the network` into `no other node has reported holding anything else`; the old wording claimed the whole network on the word of whoever happened to answer. `nothing to compare it with` also says what it actually means now, "announced to nobody, delegated to you alone and allowed to nobody", because the other cause it used to name is compared rather than skipped.
- The `RESTORE.md` that rides inside every archive said `rad-backup restore` "refuses to continue" when another node holds signed refs this copy does not have. It does not: it compares after restoring, names those repositories and exits 3. A false promise of protection, in the file somebody reads in the middle of a recovery. `move`'s parting instructions and `words --restore`'s carried the same shape of overclaim and now say what the commands do.
- `restore` no longer names one repository both as at risk and as one to push first. They give opposite instructions, and `rad sync --announce` came first in the output: the reader was handed the command that publishes the fork before reaching the paragraph telling them not to write at all.
- `restore` no longer says "no other node has reported holding signed refs of yours missing here" over a run where most repositories were never compared. The sentence is about the whole home, and it was printed directly under the line saying how many could not be asked about.
- `restore` no longer reports a peer as holding refs this copy lacks because `git` failed for a reason of its own. A failure was read as the peer's head being unresolvable if the archived head could reach itself, and `git merge-base --is-ancestor A A` answers that on equality without opening a single object, so any repository at all passed the test. It now looks for both objects: a pack that arrived truncated makes every object in it answer "absent", so a store that cannot produce your own archived head either is a broken store rather than a network holding something else.
- `restore` no longer tells you to push a repository the network may be ahead of. A node the archive recorded as behind was behind on the day the backup was taken, and heartwood writes nothing when a peer still agrees, so nothing corrects that row: acting on it printed `rad sync --announce`, which is the command that publishes the fork, over a copy months out of date. "Push this first" is now claimed only on a row that arrived during the restore, told apart from the rest by reading the home's node database before the node starts, which for an ordinary archive is the machine's own table and for a `--with-node-db` one is the archive's. A row that was already there is still reported, as `holds work no node had when the archive was taken`, which keeps the true half and drops the instruction; `--json` gains `aheadOfAStaleRecord`. The other reading of such a row, a node holding refs signed with your key that are not here, is taken from rows of any age, because that one cannot decay.
- `restore` names the repositories that could not be compared, instead of counting them and leaving the reader to find out which with `--json`.
- `restore` says when repositories were left uncompared because no node can ever hold them. A home of only private repositories used to get a report that mentioned the network nowhere at all. `--json` gains `nothingToCompare`.
- `restore` compares a repository the archive holds no signed refs of yours for, instead of writing it off. A repository you first wrote in after the backup was taken has none in the archive and every reason to have them on a seed, and it was skipped on that basis, reported as "nothing to compare", and the run exited 0: the exact case this command exists for, answered with the one sentence that means safe to write. It is fetched like any other now, and a node reporting that it holds signed refs of yours for it is the fork hazard, at exit 3.
- `rad backup restore --replay-policies` on a machine without `rad` no longer exits 4 over a home it has just rewritten. Exit 4 means the run stopped because going on would have been unsafe, and nothing was written; the flag asked for `rad` from inside the replay, which runs after the identity, the databases and every repository are on disk, and it skipped the state record on the way out so the home did not even know where it came from. The check now happens before a byte is written.
- `restore` no longer reads a repository id it cannot match as a network holding nothing. Every lookup missing is indistinguishable from every node agreeing, and the second is the sentence that reassures, so a heartwood that spells the ids in its own tables differently would have had every restore on earth report a clean home. It now says the spelling is one this build does not recognise and compares nothing.
- Counts that read wrong at one. "1 repository hold work...", "1 repository bundles open", "1 repository is ... so no node can hold them", and `verify`'s "2 problem(s)".
- `restore` no longer reassures you on the strength of a table it could not read. Heartwood renaming the sync status table comes back as an empty record, and an empty record is the sentence "no other node has reported holding anything else": every repository in the home would have been reported that way, over a comparison that never happened. It now says the schema has moved on and compares nothing.
- A restore that gave up before comparing anything now says so in `--json` rather than looking like a clean pass. `--no-reconcile`, no `rad` on `PATH` and a node that would not start all returned empty `atRisk`, `ahead` and `notChecked` arrays, which a script could not tell from a comparison that ran and found nothing wrong.
- A `git` that goes away mid-restore, or a file descriptor limit reached on the twelfth repository, no longer costs the report of which repositories did not come back. The failure propagated past the report, and no later command can reconstruct that list.
