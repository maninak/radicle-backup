...
- `restore` no longer reports a peer as holding refs this copy lacks because `git` failed for a reason of its own. A failure was read as the peer's head being unresolvable if the archived head could reach itself, and `git merge-base --is-ancestor A A` answers that on equality without opening a single object, so any repository at all passed the test. It now looks for the object, and only an object that is genuinely not here means anything about a peer.
- A restore whose node database cannot be read still reports which repositories did not come back. The read failure used to propagate past the report, so the run ended with no word about the repositories the archive carried and the home did not get, which no later command can reconstruct.
