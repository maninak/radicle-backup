...
- `restore` no longer tells you to push a repository the network may be ahead of. A node the archive recorded as behind was behind on the day the backup was taken, and heartwood writes nothing when a peer still agrees, so nothing corrects that row: acting on it printed `rad sync --announce`, which is the command that publishes the fork, over a copy months out of date. "Push this first" is now claimed only on a row that arrived during the restore, told apart from the rest by reading the archive's own copy of the node database before the node starts. The other reading of such a row, a node holding refs signed with your key that are not here, is kept from rows of any age, because that one cannot decay.
