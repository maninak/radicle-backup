...
- The `restore.sh` inside every archive refuses a home that has a symlink where the identity, its public key, `config.json` or a node database goes. It copies through a symlink at its destination, so a home seeded with one turned a restore into a write to wherever that link pointed, at whatever permissions that path had; and a dangling one at the key's own name is not a file, so the guard that refuses an occupied home walked straight past it.
