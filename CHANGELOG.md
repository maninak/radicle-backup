...
- `doctor` now opens the newest archive and says whether it still opens, using the passphrase or the `--identity` key on hand. An archive whose key is gone was reported as a working backup.
...
- `restore` now asks other nodes what they hold of your signed refs, instead of reading back the refs it had just written. Every restore used to report every repository "in step with the network", which is the one answer that means safe to write, because a fetch over a repository already in storage ignores your own key: the archive was being compared with itself. It now fetches, waits twenty seconds for other nodes to say what they hold, and reports `another node holds signed refs this copy does not have` at exit `3`.
- `restore` now compares repositories it used to write off: any repository you first wrote in after the backup was taken, and a private one another delegate or an allowed peer can hold. Only a repository announced to nobody, delegated to you alone and allowed to nobody is left uncompared, and the ones that were not compared are named rather than counted.
- `restore` no longer says your peer history has forked because `git` could not answer. An object git cannot read produced the same verdict as a real divergence, which is the most alarming thing this tool says. Such a repository is reported as uncompared now, and a comparison that fails no longer costs the report of which repositories did not come back at all.
- `restore`'s verdicts and `--json` fields changed with that comparison. `in step with the network`, `the network was ahead` and `diverged from the network` are gone; `not checked` splits into `could not be compared` and `nothing to compare it with`; `--json` renames `diverged` to `atRisk`, adds `ahead`, `aheadOfAStaleRecord` and `nothingToCompare`, and turns `notChecked` from a count into the list of repositories it counts. Anything matching on those strings needs updating.
- A restore that meant to compare and could not now exits `3` instead of `0`. No `rad` on `PATH`, a node that would not start and a fetch that did not work all leave the fork hazard unexamined, and a scheduled restore reporting success over that was the last anyone heard of it. `--no-reconcile` still exits `0`, because a check you declined is not a check that failed.
- `doctor` exits `3` when every check came back "could not be checked", instead of `0`. A run with no `rad`, no readable node database and no archive to open established nothing, and the exit code a monitoring probe reads called that clean. Unknowns beside real answers still exit `0`.
- `doctor --backup-dir` is now `doctor --dir`, matching `ls` and `prune`, and the checks look in it rather than only naming it in a remedy. `--backup-dir` still works.

### Fixed

- A passphrase-protected ssh key now opens an archive encrypted to it. No prompt appeared and the run said the key did not open the archive, which is false and is the worst thing to tell somebody mid-recovery. A key `age` cannot read no longer ends the run either, so handing it everything in `~/.ssh` works.
...
- A restore into a home that holds repositories but no key no longer goes ahead without asking. A home counted as empty when its key had been retired by an earlier `move`, deleted, or merely could not be read, and the restore then rewound every repository's refs, other peers' namespaces included. Both `restore --force` and `words --restore` read a home that way.
- A home whose `keys` directory could not be listed all the way through is no longer read as one holding no key an earlier `move` retired, which is the answer that lets a restore go ahead over somebody's displaced key.
- A home displaced twice keeps both notes about it. `DISPLACED.txt` was overwritten, so a second `restore --force` over another identity left two retired keys and one paragraph naming only the newer, and the older key was the one nobody could place. It also lands at the mode a home's own documents have, rather than at whatever the umask was.
...
- The `RESTORE.md` inside every archive, `move`'s parting instructions and `words --restore` all said `rad-backup restore` "refuses to continue" when the histories have diverged. It does not: it compares after restoring, names those repositories and exits `3`. All three say what the command does now.
...
- `restore --replay-policies` on a machine without `rad` now refuses before anything is written. It used to exit `4`, the code that means nothing was written, over a home it had already rewritten, and skip the state record on the way out, so the home did not know where it came from.
- A repository whose `packed-refs` cannot be read is named rather than treated as somebody else's. With `rad` also unavailable, `--repos mine` wrote an archive missing it and exited `0`.
...
- A dry run says when part of storage could not be measured. The estimate is meant to run high, and an unreadable directory silently counted as zero was the one thing making it run low.
...
- A restore refuses a home whose `keys`, `node` or `storage` is a symlink, or whose `storage` holds a symlink at a repository's own name. Everything that writes walks through a link standing at a directory, so a `keys` aimed at a directory somebody else owns took the private key out of the home and left it there, and a `storage` aimed at one took every repository, private ones included. It is asked before a `--words` restore too, which writes a key without reading an archive at all, and asked again after the archive is unpacked and once more before the repositories go in, rather than trusting an answer from minutes earlier. `--force` does not lift it: force is permission to overwrite what is in the home, never permission to write outside it.
- The `restore.sh` inside every archive refuses the same home, and refuses a symlink at the identity, its public key, `config.json` or a node database as well: it copies with `cp`, which writes straight through one, where `restore` replaces the link instead. A dangling link at the key's own name is not a file, so the guard that refuses an occupied home used to walk straight past it. The blocks `RESTORE.md` gives somebody to paste refuse such a home too, both the one that puts the identity back and the one that puts the repositories back.
- `words --restore` writes the public key the way it writes the key beside it, so a symlink standing at `keys/radicle.pub` is replaced rather than followed and truncated, and the file lands at `0644` rather than at whatever the umask allowed.
...
- A restore says when the git it is using checks nothing inside a bundle. Both readers pass `fetch.fsckObjects`, which is the whole of what stops a bundle writing a tree entry named `.git` into storage, and git consults it on a bundle only from 2.46: every older git accepted the setting and ignored it. `restore` and the shipped `restore.sh` both say so now, and name the git that runs the check. Both also read a release-candidate version such as `2.45-rc1` as one they cannot tell about, rather than printing a shell error at somebody mid-recovery.
- `schedule` refuses a home or output path with a line break in it, instead of writing it into the file systemd reads a setting at a time. A directory whose name carries a newline is legal, and everything after it became a second setting in a file an unattended timer reads.
- The `keys/` directory a recovery sheet rebuilds, and the state directory every run keeps its record in, are owner-only rather than whatever the umask gave. The files inside were already owner-only; the names were the leak, one per DID this machine holds a record for.
- `schedule --status` no longer leaves a copy of the archive passphrase in freed memory. It asks systemd for its whole environment to find out whether the passphrase is kept there, and that answer was read into an ordinary buffer.
- `words --restore` no longer leaves partial copies of the 24 words in freed memory. The line they are typed into started empty and grew as it was read, and every growth left what had been read so far behind, where the wiping around the finished buffer never reaches.
