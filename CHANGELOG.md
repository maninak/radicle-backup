...
- A restore that meant to compare and could not now exits `3` instead of `0`. No `rad` on `PATH`, a node that would not start and a fetch that did not work all end with the fork hazard unexamined, and a scheduled restore reporting success over that was the last anyone heard about it. `--no-reconcile` still exits `0`, because a check that was declined is not a check that failed.
- `doctor` exits `3` when every check came back "could not be checked", instead of `0`. A run with no `rad`, no readable node database and no archive to open established nothing, and the exit code a monitoring probe reads said the posture was clean. Unknowns beside real answers still exit `0`.
...
- A `move` no longer fails over the note it leaves beside the retired key. The key is renamed first, so a note that could not be read or written ended the run with an error saying the move failed, over a machine whose key was retired, and withheld the one line naming the file it went to. Both failures are now said out loud and the move finishes; a note that is there and unreadable is left exactly as it is rather than written over.
...
- A home whose `keys` directory could not be listed all the way through is no longer read as one holding no key an earlier `move` retired. The listing error was dropped and the entries behind it counted as absent, which is the answer that lets a restore go ahead over somebody's displaced key.
...
- A repository config out of an archive can no longer make `git` run a command. It was copied into `storage/<rid>/config` verbatim, where `core.pager`, `core.fsmonitor` and `remote.<name>.url = ext::sh -c ...` are each run on an ordinary operation in that repository. A restore now carries over the node's name and DID and nothing else, because everything else a storage config holds is what `git init` works out about the disk it is running on, and both `rad-backup restore` and the `restore.sh` inside every archive name what they left behind. A config git cannot parse costs those two settings rather than the repository. The same two, read the same way, in this tool, in the shipped script and in the commands `RESTORE.md` gives you to paste, and `ci/pins.sh` holds the three lists to each other.
...
- The `restore.sh` inside every archive reads a release-candidate git without falling over. `git version 2.45-rc1` took the numeric branch on its leading digits, the shell answered `[ 46-rc0 -lt 46 ]` with its own "Illegal number" in front of somebody mid-recovery, and the warning that this git writes bundles unchecked was the one thing it did not say. Both readers now answer "could not be read" to it.
...
- `words --restore` no longer leaves partial copies of the 24 words on the heap. The line they are typed into started empty and grew as it was read, and every growth freed a block still holding what had been read so far, which the wiping around the finished buffer never reaches.
